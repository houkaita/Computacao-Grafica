...
def posicao(M=4, N=4):
    ...
    for i in range(N):
        ...
        for j in range(M):
            ...
            posicao.append(y)
            posicao.append(z)
            posicao.append(u)  # Coordenada de textura U
            posicao.append(v)  # Coordenada de textura V
    return posicao
...
def malha():
    ...
    GL.glBufferData(GL.GL_ARRAY_BUFFER, len(aPosicao) * aPosicao.itemsize, ctypes.c_void_p(aPosicao.buffer_info()[0]),
                    GL.GL_STATIC_DRAW)
    GL.glVertexAttribPointer(0, 3, GL.GL_FLOAT, GL.GL_FALSE, 5 * aPosicao.itemsize, ctypes.c_void_p(0))
    GL.glVertexAttribPointer(1, 2, GL.GL_FLOAT, GL.GL_FALSE, 5 * aPosicao.itemsize, ctypes.c_void_p(3 * aPosicao.itemsize))
    ...
    GL.glBufferData(GL.GL_ELEMENT_ARRAY_BUFFER, len(aIndices) * aIndices.itemsize,
                    ctypes.c_void_p(aIndices.buffer_info()[0]), GL.GL_STATIC_DRAW)
