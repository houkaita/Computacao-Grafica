# Trabalho feito com Felipe Junior e Lucas Rocha
...
def cubo():
    ...
    # Configurando as coordenadas dos vertices do cubo a fim de "conectar" os triangulos
    posicao = array('f',[
        *A, *B, *C, *A, *C, *D,
        *E, *F, *G, *E, *G, *H,
        *D, *C, *G, *D, *G, *H,
    ...
    # Nessa parte definimos a divisão de cada parte do dado da imagem
    # A imagem tem 6 quadradinhos e precisamos dividir-los demilitando a area de cada um deles. 1/3 de tamanho para um quadrado para a horizontal (eixo x)  e 1/2 de tamanho na vertical (eixo y), 
    
    #Perceba na definição dos pontos de textura que cada underline (_) representa:
    #  y = 0 (sem underline)
    #  y = 1/2 (um underline)
    #  y = 1 (dois underlines)
    f1 = (*p1, *p2, *p2_, *p1, *p2_, *p1_) # 4
    f2 = (*p1_, *p2_, *p2__, *p1_, *p2__, *p1__) # 1

    f3 = (*p3, *p3_, *p2_, *p3, *p2_, *p2) # 5
    f4 = (*p3_, *p3__, *p2__, *p3_, *p2__, *p2_) #2

    f5 = (*p4, *p4_, *p3_, *p4, *p3_, *p3) #3
    f6 = (*p4_, *p4__, *p3__, *p4_, *p3__, *p3_) #6
    texture = array('f',[
        *f1,
        *f2,
        *f3,
